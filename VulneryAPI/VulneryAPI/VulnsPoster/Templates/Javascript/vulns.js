...
function setModalInfraData(InfraVuln, isEdit, buttonId, action, buttonCSV){
    ...
    if(isEdit){
        ...
        if(buttonCSV){
            ...
            let btnUploadCSV = `<button type='button' class='btn btn-dark' id=${buttonCSV}>Upload CSV</button>`;
...
function addSaveButtonToModal(modal, buttonId, actionVuln){
    if($(modal).find('.modal-footer').children().length==0){
        let saveButton = `<button type="button" class="btn btn-dark" id=${buttonId}>${actionVuln} vuln</button>`
