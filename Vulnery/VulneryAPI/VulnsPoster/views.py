...
def postVulns(request):
    if request.method == "POST":
        try:
            vulnJSON = vulnsPoster.recieveSASTVuln(request)
            processedVuln = vulnsPoster.processSASTVuln(vulnJSON)
            vulnsPoster.uploadSASTVulnIntoElastic(processedVuln)
            #Listar vulns en html
            return render(request, "vulns.html", {'list':vulnJSON})
        except:
            pass #si la vuln da error sumar a vulnsErrors
