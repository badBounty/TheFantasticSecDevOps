def runStage(notifier)
{
    try 
    {
        ...
        sshagent(['ssh-key-SAST-server']) 
        {
            ...
            sh "ssh -o StrictHostKeyChecking=no ${env.SAST_Server_User}@${env.SAST_Server_IP} screen -d -m ${env.SAST_Server_Repository_SAST_Path}/start.sh nobuild ${projname} ${env.Sonar_Port} ${env.SAST_Server_SSH_Port} ${env.SAST_Server_Repository_SAST_Path}"
            sh 'sleep 15m'
