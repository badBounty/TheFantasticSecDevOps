def runStage(notifier)
{
    ...
    try {
        ...
        withCredentials([usernamePassword(credentialsId: 'sonar-credentials', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')])
        {
            ...
            {
                sh "ssh -p ${env.SAST_Server_SSH_Port} -o StrictHostKeyChecking=no root@${env.SAST_Server_IP} cd /home/${env.SonarQube_Maven_Path}/ && mvn sonar:sonar -Dsonar.host.url=http://localhost:${env.Sonar_Port} -Dsonar.projectKey=${projname} -Dsonar.projectBaseDir=/home/${env.SonarQube_Maven_Path}/ -Dsonar.login=${USERNAME} -Dsonar.password=${PASSWORD} -X -DskipTests "
            }                
        }
